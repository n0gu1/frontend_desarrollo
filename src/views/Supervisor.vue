    ...
    <main class="main-content">
      ...
      <div class="orders-table-section">
        <div class="table-container">
          <table class="orders-table">
            ...
            <tbody v-if="loading && orders.length === 0">
              <tr v-for="n in 6" :key="'sk-' + n">
                <td>
                  <div class="skeleton pill" style="width:100px;height:18px;"></div>
                </td>
                <td>
                  <div class="skeleton" style="width:240px;height:18px;"></div>
                </td>
                <td>
            ...
            <tbody v-else>
              <tr v-for="o in orders" :key="o.id || o.folio">
                ...
                <td>
                  <div class="operator-cell">
                    <select v-model.number="selectedOperators[o.id]" class="filter-select" :disabled="saving[o.id]">
                      <option :value="null">— Sin asignar —</option>
                      <option v-for="op in operators" :key="op.usuarioId" :value="op.usuarioId">
                        {{ op.nickname }} ({{ op.usuarioId }})
                      </option>
                    </select>

                    <button class="btn-assign" :disabled="saving[o.id]" @click="assign(o.id, selectedOperators[o.id])"
          ...
          <div v-if="!loading && orders.length === 0" class="no-results">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <circle cx="11" cy="11" r="8" />
              <path d="m21 21-4.35-4.35" />
            </svg>
            <p>No hay órdenes con estado CRE</p>
          </div>
...
export default {
  ...
  data() {
    return {
      orders: [],                 // [{ id, folio, customerName }]
      operators: [],              // [{ usuarioId, nickname, email }]
      selectedOperators: {},      // orderId -> operadorUsuarioId (Number|null)
      assignedNames: {},          // orderId -> nickname (UX)
      saving: {},                 // orderId -> boolean
      loading: false,
      error: null,
      _refreshTimer: null
    }
  ...
  methods: {
    ...
    mapOrder(row) {
      // Soporta distintas formas del backend
    ...
    async loadOrders(limit = 200) {
      // El backend debe devolver sólo estado=CRE
    ...
    async loadOperators() {
      // Operadores reales: RolId=5 y solo activos
    ...
    async loadAssignments() {
      // Si implementaste el endpoint de asignaciones actuales
      try {
        ...
        arr.forEach(r => {
          ...
          const opId = r.operadorUsuarioId ?? r.OperadorUsuarioId ?? null
          const nick = r.operadorNickname ?? r.OperadorNickname ?? null
          this.$set(this.selectedOperators, ordenId, opId === null ? null : Number(opId))
          if (nick) this.$set(this.assignedNames, ordenId, nick)
        })
      } catch {
        // Silencioso
      }
    ...
    async reload() {
      ...
      this.error = null
      try {
        // Carga en paralelo (más rápido)
        await Promise.all([this.loadOrders(), this.loadOperators()])
        await this.loadAssignments() // opcional
      } catch (e) {
        console.error(e)
    ...
    async assign(ordenId, operadorUsuarioId) {
      if (!ordenId) return
      try {
        this.$set(this.saving, ordenId, true)
        await post(`/api/supervisor/orders/${ordenId}/assign-operator`, {
          operadorUsuarioId: operadorUsuarioId ?? null
        })
        const nick =
          this.operators.find(x => Number(x.usuarioId) === Number(operadorUsuarioId))?.nickname || '—'
        this.$set(this.assignedNames, ordenId, nick)
      } catch (e) {
        alert('No se pudo guardar la asignación')
        console.error(e)
      } finally {
        this.$set(this.saving, ordenId, false)
      }
    }
  },
  ...
  async mounted() {
    ...
    // Auto-refresco liviano cada 60s (opcional; comenta si no lo quieres)
    this._refreshTimer = setInterval(() => {
      // Sólo recarga órdenes rápidas para no bloquear UI
...
.orders-table th {
  ...
  font-weight: 600;
  font-size: .9rem;
  border-bottom: 2px solid rgba(79, 209, 197, .3);
}
